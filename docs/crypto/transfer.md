...
- [Revolut](https://revolut.com/referral/?referral-code=ivanc2xbx!OCT1-24-AR-H2) —
...
Нашёл следующие относительно дешёвые варианты:
...
- [Revolut](https://revolut.com/referral/?referral-code=ivanc2xbx!APR2-24-AR).
  В приложении есть отдельная вкладка с криптой —
  удобно, если уже пользуешься им как банком.
...
Для тру криптанов есть self-custody криптокарта, где ваши деньги лежат прямо в defi смарт-контракте
[GnosisPay](https://app.gnosispay.com/signup?rcc=579b180470d8).
Если вы не поняли, что тут написано, значит, оно вам и не надо :).
Немного замороченно устроено пополнение EURe токенов, в остальном работает как обычная карта.
Доступна только резидентам Европы.
В ApplePay добавляется с помощью курвы (curve, см. ниже).

Обычная криптокарта, где деньги лежат в виде токенов прямо на бирже есть в виде
[виртуальной и физической криптокарты Bybit](https://www.bybit.com/fiat/cards),
комиссии были в рамках 0-2%. Доступны только резидентам Европы.

Ну и последняя — криптокарта [crypto.com](https://crypto.com/app/jwfkfv37yv), практически идентичная карте bybit.

Все криптокарты не поддерживают Apple Pay, но это решается сервисом курвы [Curve](https://www.curve.com).
В нём можно создать виртуальную карту, которую можно добавить в Apple Pay.
Она будет «питаться» с другой банковской карты (вышеупомянутых криптокарт).
...
У GnosisPay так сделать не получится, каждый платёж будет налоговым ивентом.
...
Чтобы платить налоги в Европе советую пользоваться [koinly](https://koinly.io/?via=F5F3F5A2&utm_source=friend).
Он недешёвый, но дело своё делает.
Тарифы зависят от количество транзакций, от 100 до 300+ евро.
Поддерживает большинство бирж, и чейнов,
но супер редкие или свежие можно не найти (например, блокчейна SUI нет).
Для шведа он генерит файлик, который тупо заливаешь в налоговую и всё, ты молодец.

Перед этим пробовал kryptos.io — оказалось полным говном:
не смог сматчить транзакции, поддержка не отвечает неделями, в итоге плюнул на него.

Вот основные документы (для РФ и Швеции):
